...
class Article_type_m extends CI_Model 
{
	/**
	 * cid树
	 * 
	...
	public function add_type($name, $pid = 0) 
	{
		...
		if($pid == 0 || $ppid == 0 ||  $pppid == 0) {
			...
			if($this->db->insert('articles_type', $data) === FALSE) {
				return FALSE;
			}
	...
	public function edit_type($tid, $name, $pid = 0) 
	{
		...
		if($this->db->update('articles_type', $data) === FALSE) {
			return FALSE;
		}
		return TRUE;
	}
	...
	public function del_type($tid) 
	{
		...
		$this->db->where('tid', $tid);
		if($this->db->delete('articles_type') === FALSE) {
			return FALSE;
		}
		return TRUE;
	}
	...
	{
	    $sql = "SELECT * FROM `zh_articles_type` WHERE `pid` != 0 AND `pid` != 7";
